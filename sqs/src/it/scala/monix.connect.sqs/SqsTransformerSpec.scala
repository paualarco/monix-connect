...
class SqsTransformerSpec
  ...
  s"${SqsOp}.transformer() creates a transformer operator" should "transform `SendMessageRequest` to `SendMessageResponse`" in {
     // given
     val randomMessageBody: String = genMessageBody.sample.get
     val sendTransformer = SqsOp.transformer[SendMessageRequest, SendMessageResponse]
  ...
  it should "transform `ListQueuesRequest` to `ListQueuesResponse`" in {
    // given
    val sendTransformer = SqsOp.transformer[ListQueuesRequest, ListQueuesResponse]
