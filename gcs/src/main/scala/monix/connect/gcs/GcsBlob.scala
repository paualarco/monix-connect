...
import monix.connect.gcs.components.{FileIO, GcsUploader, GcsDownloader}
 ...
 */
private[gcs] final class GcsBlob(underlying: Blob)
   ...
   *   import monix.connect.gcs.GcsBlob
   *   import monix.reactive.Observable
   *
   *   val bucketName = "sampleBucket"
   *   val blobPath = "/path/to/your/blob"
   *   val blobInfo: BlobInfo = BlobInfo.newBuilder(BlobId.of(bucketName, blobPath)).build
   *   val storage: Storage = Storage.create()
   ...
   *
   *   bucket.downloadToFile("blob1", Paths.get("file.txt"))
